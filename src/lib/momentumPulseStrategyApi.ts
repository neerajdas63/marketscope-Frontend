...
function normalizeSummary(value: unknown): MomentumPulseStrategySummary {
  ...
  return {
    ...
    fresh_signal_count: asFiniteNumber(data.fresh_signal_count) ?? 0,
    stale_signal_count: asFiniteNumber(data.stale_signal_count) ?? 0,
    avg_volume_ratio: asFiniteNumber(data.avg_volume_ratio) ?? 0,
    avg_range_ratio: asFiniteNumber(data.avg_range_ratio) ?? 0,
    avg_execution_rank: asFiniteNumber(data.avg_execution_rank) ?? 0,
    avg_signal_age_minutes: asFiniteNumber(data.avg_signal_age_minutes) ?? 0,
...
function normalizeStrategyRow(value: unknown): MomentumPulseStrategyRow | null {
  ...
  return {
    ...
    refresh_time: asOptionalString(row.refresh_time) ?? "",
    market_data_last_updated:
      asOptionalString(row.market_data_last_updated) ?? "",
    signal_age_minutes: asNullableNumber(row.signal_age_minutes),
    signal_freshness: asOptionalString(row.signal_freshness) ?? "UNKNOWN",
    signal_is_fresh: asOptionalBoolean(row.signal_is_fresh) ?? null,
