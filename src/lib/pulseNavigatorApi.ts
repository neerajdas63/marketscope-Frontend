...
const VALID_PRESETS: PulseNavigatorPreset[] = [
  "balanced",
  "safe",
  "aggressive",
  "fo_focus",
];
const VALID_DIRECTIONS: PulseNavigatorDirection[] = [
  "LONG",
  "SHORT",
  "NEUTRAL",
];
const VALID_DIRECTION_FILTERS: PulseNavigatorDirectionFilter[] = [
  "ALL",
  "LONG",
  "SHORT",
];
...
function asPreset(
  value: unknown,
  fallback: PulseNavigatorPreset,
): PulseNavigatorPreset {
  return typeof value === "string" &&
    VALID_PRESETS.includes(normalizeToken(value) as PulseNavigatorPreset)
...
function asDirection(value: unknown): PulseNavigatorDirection {
  return typeof value === "string" &&
    VALID_DIRECTIONS.includes(value.toUpperCase() as PulseNavigatorDirection)
...
function asDirectionFilter(
  value: unknown,
  fallback: PulseNavigatorDirectionFilter,
): PulseNavigatorDirectionFilter {
  return typeof value === "string" &&
...
function asActionabilityLabel(
  ...
  if (typeof value === "string") {
    ...
    if (
      VALID_ACTIONABILITY.includes(
        normalized as PulseNavigatorActionabilityLabel,
      )
    ) {
...
function normalizeHighlight(
  value: unknown,
): PulseNavigatorHeroHighlight | null {
  ...
  const trend =
    asOptionalString(record.pulse_trend_label) ??
    asOptionalString(record.trend_label);
  const score =
    asFiniteNumber(record.momentum_pulse_score) ?? asFiniteNumber(record.score);
  const extras = [
    direction,
...
function normalizeStock(value: unknown): PulseNavigatorStock | null {
  ...
  const warningFlags =
    asStringArray(row.warning_flags).length > 0
  ...
  return {
    symbol,
    sector:
      asOptionalString(row.sector) ?? asOptionalString(row.group) ?? "Unknown",
    direction: asDirection(row.direction ?? row.inferred_direction),
    momentum_pulse_score:
      asFiniteNumber(row.momentum_pulse_score) ??
      asFiniteNumber(row.score) ??
      0,
    session_leader_score: asFiniteNumber(row.session_leader_score) ?? 0,
    direction_confidence:
      asFiniteNumber(row.direction_confidence) ??
      asFiniteNumber(row.direction_conf) ??
      0,
    actionability_label: asActionabilityLabel(
    ...
    ui_tags: (asStringArray(row.ui_tags).length > 0
      ? asStringArray(row.ui_tags)
      : asStringArray(row.tags)
    ).slice(0, 4),
    change_pct:
      asFiniteNumber(row.change_pct) ?? asFiniteNumber(row.pct_change) ?? 0,
    ...
    latest_bar_time:
      asOptionalString(row.latest_bar_time) ??
      asOptionalString(row.bar_time) ??
      "--",
...
function normalizeDiscoverBuckets(
  value: unknown,
): PulseNavigatorDiscoverBucket[] {
  if (Array.isArray(value)) {
    return value
      .map((bucket, index) => {
        ...
        const key =
          asOptionalString(bucketRecord.key) ??
          asOptionalString(bucketRecord.slug) ??
          `bucket_${index}`;
      ...
      .filter(
        (bucket): bucket is PulseNavigatorDiscoverBucket => bucket !== null,
      );
  }
...
function normalizeSectorEntry(
  value: unknown,
  fallbackSector: string,
): PulseNavigatorSectorEntry | null {
  if (!value || typeof value !== "object") {
    return null;
  }
  ...
  const fallbackTopStocks = [bestStock, challenger, laggard].filter(
    (stock): stock is PulseNavigatorStock => stock !== null,
  );
  const topStocks = (
    explicitTopStocks.length > 0 ? explicitTopStocks : fallbackTopStocks
  ).filter(
    (stock, index, stocks) =>
      stocks.findIndex((candidate) => candidate.symbol === stock.symbol) ===
      index,
  );
  const sectorDirectionRaw =
    asOptionalString(sector.sector_direction) ??
    asOptionalString(sector.direction);
  ...
  return {
    sector:
      asOptionalString(sector.sector) ??
      asOptionalString(sector.name) ??
      fallbackSector,
    ...
    laggard,
    sector_score:
      asFiniteNumber(sector.sector_score) ??
      asFiniteNumber(sector.score) ??
      bestStock?.momentum_pulse_score ??
      null,
    market_relative_score:
      asFiniteNumber(sector.market_relative_score) ??
      asFiniteNumber(sector.relative_score) ??
      null,
    average_change_pct:
      asFiniteNumber(sector.average_change_pct) ??
      asFiniteNumber(sector.avg_change_pct) ??
      null,
    candidate_count:
      asFiniteNumber(sector.candidate_count) ??
      asFiniteNumber(sector.stock_count) ??
      null,
...
function normalizeSectorEntries(value: unknown): PulseNavigatorSectorEntry[] {
  ...
  return Object.entries(value as Record<string, unknown>)
    .map(([sectorName, sectorValue]) =>
      normalizeSectorEntry(sectorValue, sectorName),
    )
    .filter((entry): entry is PulseNavigatorSectorEntry => entry !== null);
}
...
export function normalizePulseNavigatorResponse(
  ...
  const benchmarkChangePct =
    asFiniteNumber(data.benchmark_change_pct) ??
    asFiniteNumber(data.benchmark_pct_change);
  const marketModeHighlight = normalizeHighlight(
    heroData.market_mode ?? data.market_mode,
  );
  const benchmarkLabel = asOptionalString(data.benchmark_label) ?? "Benchmark";
  const benchmarkValue =
    asOptionalString(data.benchmark_value) ??
    (benchmarkChangePct !== undefined
      ? formatSignedPercent(benchmarkChangePct, 2)
      : "--");
  ...
  const discoverSource =
    tab === "discover"
      ? data
      : tabsData.discover && typeof tabsData.discover === "object"
        ? (tabsData.discover as Record<string, unknown>)
        : {};
  const leadersSource =
    tab === "leaders"
      ? data
      : tabsData.leaders && typeof tabsData.leaders === "object"
        ? (tabsData.leaders as Record<string, unknown>)
        : {};
  const freshSource =
    tab === "fresh"
      ? data
      : tabsData.fresh && typeof tabsData.fresh === "object"
        ? (tabsData.fresh as Record<string, unknown>)
        : {};
  const sectorsSource =
    tab === "sectors"
      ? data
      : tabsData.sectors && typeof tabsData.sectors === "object"
        ? (tabsData.sectors as Record<string, unknown>)
        : {};

  const discoverBuckets = normalizeDiscoverBuckets(
    discoverSource.buckets ?? discoverSource.groups ?? discoverSource.sections,
  );
  const discoverStocks = discoverBuckets.flatMap((bucket) => bucket.stocks);
  const legacyFreshStocks = normalizeStockList(
    Array.isArray(freshSource.stocks)
      ? freshSource.stocks
      : Array.isArray(freshSource.data)
        ? freshSource.data
        : [],
  );
  ...
  const hasExplicitLeaders =
    explicitLeaderLongs.length > 0 || explicitLeaderShorts.length > 0;
  const hasExplicitFresh =
    explicitFreshLongs.length > 0 || explicitFreshShorts.length > 0;
  const leaderFallback =
    discoverStocks.length > 0 ? discoverStocks : legacyFreshStocks;
  ...
  return {
    status: normalizeStatus(
      data.status ??
        data.state ??
        (asOptionalBoolean(data.ready) ? "ready" : undefined),
    ),
    last_updated:
      asOptionalString(data.last_updated) ??
      asOptionalString(data.updated_at) ??
      "",
    ...
    hero: {
      ...
      leader_long: normalizeHighlight(
        heroData.leader_long ?? heroData.best_long,
      ),
      leader_short: normalizeHighlight(
        heroData.leader_short ?? heroData.best_short,
      ),
      fresh_long: normalizeHighlight(
        heroData.fresh_long ?? heroData.best_fresh,
      ),
      fresh_short: normalizeHighlight(heroData.fresh_short),
      strongest_sector: normalizeHighlight(heroData.strongest_sector),
      leaders_overview: normalizeHighlight(
        heroData.leaders_overview ?? data.leaders_overview,
      ),
    },
    tabs: {
      discover: { buckets: discoverBuckets },
      leaders: {
        longs: hasExplicitLeaders
          ? explicitLeaderLongs
          : splitLeaderFallback.longs,
        shorts: hasExplicitLeaders
          ? explicitLeaderShorts
          : splitLeaderFallback.shorts,
      },
      fresh: {
        longs: hasExplicitFresh ? explicitFreshLongs : splitFreshFallback.longs,
        shorts: hasExplicitFresh
          ? explicitFreshShorts
          : splitFreshFallback.shorts,
      },
...
export async function fetchPulseNavigatorData(
  ...
  const response = await apiFetch(`/pulse-navigator?${params.toString()}`, {
    signal,
  });
