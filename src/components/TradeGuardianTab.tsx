...
const OPEN_STATUSES: TradeGuardianTradeStatus[] = [
  "pending",
  "active",
  "t1_hit",
];
const CLOSED_STATUSES: TradeGuardianTradeStatus[] = [
  "t2_hit",
  "sl_hit",
  "closed_manual",
  "cancelled",
...
function sortAlerts(alerts: TradeGuardianAlert[]) {
  return [...alerts].sort((left, right) => {
    const leftCritical =
      left.alert_type === "stop_loss_hit" || left.alert_type === "sl_hit"
        ? 1
        : 0;
    const rightCritical =
      right.alert_type === "stop_loss_hit" || right.alert_type === "sl_hit"
        ? 1
        : 0;
    ...
    const rightUnresolved =
      !right.acknowledged_at && !right.resolved_at ? 1 : 0;
    if (leftUnresolved !== rightUnresolved) {
      return rightUnresolved - leftUnresolved;
    }

    return (right.last_sent_at || right.first_triggered_at).localeCompare(
      left.last_sent_at || left.first_triggered_at,
    );
  });
}
  ...
  return (
    <article className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span
              className={`rounded-full border px-2.5 py-1 text-[11px] font-bold ${isShort ? "border-rose-500/30 bg-rose-500/10 text-rose-100" : "border-emerald-500/30 bg-emerald-500/10 text-emerald-100"}`}
            >
              {trade.direction}
            </span>
            <span
              className={`rounded-full border px-2.5 py-1 text-[11px] font-bold ${statusMeta.className}`}
            >
              {statusMeta.label}
            </span>
          </div>
          <div className="mt-2 text-sm text-slate-400">
            {trade.latest_alert || "No alert emitted yet."}
          </div>
        </div>
        <div className="text-right text-xs text-slate-500">
          <div>Created {formatTimestamp(trade.created_at)}</div>
          <div className="mt-1">
            Updated {formatTimestamp(trade.updated_at)}
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        ...
        <PriceCell
          label="Last Price"
          value={trade.last_price}
          highlight={isShort ? "#fca5a5" : "#7dd3fc"}
        />
        <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
          <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">
            Lifecycle
          ...
          <div className="mt-1 text-xs font-semibold text-slate-300">
            Act: {formatTimestamp(trade.activated_at)}
          </div>
          <div className="mt-1 text-xs font-semibold text-slate-400">
            Close: {formatTimestamp(trade.closed_at)}
          </div>
        </div>
      </div>
    ...
    <article className={`rounded-2xl border p-4 ${tone.cardClass}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className={`rounded-full border px-2.5 py-1 text-[11px] font-bold ${tone.badgeClass}`}
            >
              {tone.label}
            </span>
            {repeating ? (
          ...
          <div className="mt-3 text-base font-bold text-slate-50">
            {alert.message || "Alert message unavailable"}
          </div>
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-400">
            <span>{alert.related_symbol || "Unknown symbol"}</span>
            <span className="rounded-full border border-slate-700 px-2 py-0.5">
              {alert.related_direction ?? "--"}
            </span>
            <span
              className={`rounded-full border px-2 py-0.5 ${statusMeta.className}`}
            >
              {statusMeta.label}
            </span>
          </div>
        ...
        <div className="text-right text-xs text-slate-400">
          ...
          <div className="mt-1">
            Last price {formatCurrency(alert.last_price)}
          </div>
        </div>
      </div>
  ...
  if (!detail?.trade) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-6 text-sm text-slate-400">
        Select a trade from Active Trades or Closed Trades to inspect the
        backend timeline and alert history.
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span
                className={`rounded-full border px-2.5 py-1 text-[11px] font-bold ${trade.direction === "SHORT" ? "border-rose-500/30 bg-rose-500/10 text-rose-100" : "border-emerald-500/30 bg-emerald-500/10 text-emerald-100"}`}
              >
                {trade.direction}
              </span>
              <span
            ...
            <div className="mt-2 text-sm text-slate-400">
              {trade.notes || "No notes attached to this trade."}
            </div>
          </div>
          <div className="text-right text-xs text-slate-400">
            <div>Created {formatTimestamp(trade.created_at)}</div>
            ...
            <div className="mt-1">
              Closed {formatTimestamp(trade.closed_at)}
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
            ...
            <div className="mt-1 text-xs text-slate-400">
              Activated {formatTimestamp(trade.activated_at)}
            </div>
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-950/70 p-5">
        ...
        <div className="mt-3 space-y-3">
          {sortAlerts(detail.alerts).length > 0 ? (
            sortAlerts(detail.alerts).map((alert) => (
              <AlertCard
                key={alert.id}
                alert={alert}
...
export function TradeGuardianTab() {
  ...
  const [summary, setSummary] = useState<TradeGuardianDashboardSummary>(() =>
    normalizeTradeGuardianDashboard({}),
  );
  ...
  const activeTrades = useMemo(
    () => trades.filter((trade) => OPEN_STATUSES.includes(trade.status)),
    [trades],
  );
  const closedTrades = useMemo(
    () => trades.filter((trade) => CLOSED_STATUSES.includes(trade.status)),
    [trades],
  );
  ...
  useEffect(() => {
    ...
    async function loadWorkspace(initial = false) {
      ...
      const [summaryResult, tradesResult, alertsResult] =
      ...
      const nextAlerts =
        alertsResult.status === "fulfilled" ? alertsResult.value : alerts;
      ...
      if (
        ...
        setError(
          summaryResult.reason instanceof Error
            ...
            : "Unable to load Trade Guardian workspace",
        );
      } else if (
        summaryResult.status === "rejected" ||
        tradesResult.status === "rejected" ||
        ...
        setError(
          "Trade Guardian loaded partially. Some backend sections are temporarily unavailable.",
        );
      }
  ...
  async function refreshWorkspaceNotice(message: string) {
    ...
    setSummary(
      normalizeTradeGuardianDashboard(summaryPayload, nextTrades, nextAlerts),
    );
  }
  ...
  async function handleCreateTrade(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await refreshWorkspaceNotice(
        `Trade created for ${payload.symbol}. Backend monitoring is now the source of truth.`,
      );
  ...
  async function handleCloseTrade(tradeId: string) {
    if (
      !window.confirm(
        "Close this trade? The backend will mark it closed_manual and stop live tracking for the open position.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await refreshWorkspaceNotice(
        "Trade closed manually. Backend state refreshed.",
      );
      setView("closed");
    } catch (closeError) {
      setError(
  ...
  async function handleAcknowledge(alertId: string) {
    ...
    try {
      ...
      await refreshWorkspaceNotice(
        "Alert acknowledged. Repeated reminders should now stop on the backend.",
      );
    } catch (ackError) {
      setError(
  ...
  async function handleMonitorRefresh() {
    ...
    try {
      ...
      await refreshWorkspaceNotice(
        "Manual monitor refresh completed. Latest backend trade and alert state reloaded.",
      );
    } catch (monitorError) {
      setError(
        monitorError instanceof Error
          ? monitorError.message
          : "Unable to trigger manual monitor refresh",
      );
    } finally {
      setMonitoring(false);
    }
  ...
  async function handleTestTelegram() {
    ...
    try {
      ...
      setNotice(
        "Test Telegram alert requested. Confirm delivery from the backend alert channel.",
      );
    } catch (telegramError) {
      setError(
        telegramError instanceof Error
          ? telegramError.message
          : "Unable to send test Telegram alert",
      );
    } finally {
  ...
  function updateForm<K extends keyof TradeFormState>(
    key: K,
  ...
  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center text-lg text-cyan-300">
        Loading Trade Guardian...
      </div>
    );
  }
    ...
    >
      ...
      <div className="p-4">
        <section className="rounded-3xl border border-slate-800 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.14),transparent_32%),linear-gradient(180deg,rgba(9,15,28,0.95),rgba(9,15,28,0.85))] p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-[11px] font-bold uppercase tracking-[0.22em] text-cyan-300/80">
                Live Trade Tracker
              </div>
              <h1 className="mt-1 text-3xl font-black text-slate-50">
                Trade Guardian
              </h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
                Manual trade entry with backend-owned live monitoring, Telegram
                alerts, and state transitions for entry, stop loss, and targets.
                The frontend only renders backend truth so the risk state stays
                impossible to miss.
              </p>
            </div>
          ...
          <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
            <StatCard
              ...
              detail="Entry not hit yet"
              accent="#fcd34d"
            />
            <StatCard
              label="Active Trades"
              value={String(summary.active_count)}
              detail="Backend monitoring live"
              accent="#86efac"
            />
            <StatCard
              label="Closed Trades"
              value={String(summary.closed_count)}
              detail="T2, SL, manual, or cancelled"
              accent="#cbd5e1"
            />
            <StatCard
              label="Open Alerts"
              value={String(summary.unacknowledged_alert_count)}
              detail="Needs user attention"
              accent="#fda4af"
            />
            <StatCard
              label="Repeating Alerts"
              value={String(summary.repeating_alert_count)}
              detail="Still escalating until acknowledged"
              accent="#fbbf24"
            />
            <StatCard
              label="Monitor"
              ...
              detail={
                summary.last_monitor_run_at
                  ? `Last run ${formatTimestamp(summary.last_monitor_run_at)}`
                  : "Waiting for first backend run"
              }
              accent={
                summary.telegram_enabled === false ? "#fda4af" : "#7dd3fc"
              }
            />
          </div>
        </section>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <SectionButton
            active={view === "alerts"}
            label={`Alert Center (${sortedAlerts.length})`}
            icon={Bell}
          ...
          <SectionButton
            active={view === "closed"}
            label={`Closed / Review (${closedTrades.length})`}
            icon={ShieldAlert}
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Create the trade manually. After submission, the backend owns
                  state progression, alerting, and reminder cadence.
                </p>
              </div>
            ...
            >
              ...
              <label className="flex flex-col gap-2 text-sm font-semibold text-slate-200">
                ...
                <input
                  ...
                  onChange={(event) =>
                    updateForm("entry_price", event.target.value)
                  }
                  type="number"
                  step="0.01"
                  className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-400/50"
              ...
              <label className="flex flex-col gap-2 text-sm font-semibold text-slate-200">
                ...
                <input
                  ...
                  onChange={(event) =>
                    updateForm("stop_loss", event.target.value)
                  }
                  type="number"
                  step="0.01"
                  className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-400/50"
              ...
              <label className="flex flex-col gap-2 text-sm font-semibold text-slate-200">
                ...
                <input
                  value={form.target_1}
                  onChange={(event) =>
                    updateForm("target_1", event.target.value)
                  }
                  type="number"
                  step="0.01"
                  className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-400/50"
              ...
              <label className="flex flex-col gap-2 text-sm font-semibold text-slate-200">
                ...
                <input
                  value={form.target_2}
                  onChange={(event) =>
                    updateForm("target_2", event.target.value)
                  }
                  type="number"
                  step="0.01"
                  className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-400/50"
              ...
              <label className="md:col-span-2 xl:col-span-4 flex flex-col gap-2 text-sm font-semibold text-slate-200">
                ...
                <textarea
                  ...
                  rows={4}
                  className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-400/50"
                  placeholder="Why this trade exists, what invalidates it, and how you want the reminder cadence interpreted."
                />
              </label>
              <div className="md:col-span-2 xl:col-span-4 flex items-center justify-between gap-4 flex-wrap">
                <div className="text-xs text-slate-400">
                  Pending means the trade exists before entry. Active, T1, T2,
                  SL, and alert lifecycles all come from the backend.
                </div>
                <button
                  type="submit"
                  disabled={submitting}
                  className="rounded-xl border border-cyan-400/30 bg-cyan-500/10 px-4 py-2 text-sm font-semibold text-cyan-100 hover:bg-cyan-500/20 disabled:cursor-not-allowed disabled:opacity-50"
                >
          ...
          <section className="mt-4 space-y-4">
            {activeTrades.length > 0 ? (
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-8 text-center text-sm text-slate-400">
                No pending or active trades are currently tracked by the
                backend. Add a trade to start the live guardian workflow.
              </div>
            )}
          </section>
        ) : null}
            ...
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
              <div>
                ...
                <div className="mt-1 text-sm text-slate-200">
                  {selectedTrade
                    ? `${selectedTrade.symbol} • ${selectedTrade.direction} • ${statusStyles[selectedTrade.status].label}`
                    : "No trade selected"}
                </div>
              </div>
              <select
              ...
              >
                {trades.map((trade) => (
                  <option key={trade.id} value={trade.id}>
                    {trade.symbol} • {statusStyles[trade.status].label}
                  </option>
                ))}
              </select>
            </div>
            {detailLoading ? (
              <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-8 text-center text-sm text-cyan-200">
                Loading backend trade detail...
              </div>
            ) : (
        ...
        <section className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-sm leading-6 text-slate-400">
          <div className="flex items-center gap-2 text-slate-200">
            <CircleAlert size={16} /> Backend change context
          </div>
          <div className="mt-2">
            Trade Guardian state comes from the backend live tracker and
            Telegram system. A trade can move through pending, active, target,
            stop loss, manual close, or cancellation based on monitored market
            state. The frontend does not promote or suppress those transitions
            on its own.
          </div>
          <div className="mt-2">
            Stop loss alerts are rendered as the highest-priority visual state.
            Target alerts remain visible until acknowledged, and repeated alerts
            explicitly show that they are still escalating.
          </div>
        </section>
      </div>
    </div>
