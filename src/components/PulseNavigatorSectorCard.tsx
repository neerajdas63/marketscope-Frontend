    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 mt-3">
        ...
        <Stat
          ...
          accent={stock.change_pct >= 0 ? "#86EFAC" : "#FCA5A5"}
        />
      </div>
    </div>
  );
...
export function PulseNavigatorSectorCard({
  sector,
}: {
  sector: PulseNavigatorSectorEntry;
}) {
  const bestStock = sector.best_stock ?? sector.leader;
  const topStocks = (
    sector.top_stocks.length > 0
      ? sector.top_stocks
      : [bestStock, sector.challenger, sector.laggard].filter(
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 flex-wrap mt-4">
        <SummaryChip
          label="Sector Score"
          value={formatNumber(sector.sector_score, 1)}
          accent="#F8FAFC"
        />
        <SummaryChip
          label="Relative Score"
          value={formatNumber(sector.market_relative_score, 1)}
          accent="#CFFAFE"
        />
        <SummaryChip
          label="Breadth"
          value={
            sector.candidate_count !== null
              ? `${sector.candidate_count} names`
              : "--"
          }
        ...
        <SummaryChip
          label="Avg Change"
          value={formatSignedPercent(sector.average_change_pct, 1)}
          accent={
            sector.average_change_pct !== null && sector.average_change_pct >= 0
              ? "#86EFAC"
              : "#FCA5A5"
          }
        />
          ...
          {bestStock ? (
            <>
                ...
                <div className="flex items-center gap-2 flex-wrap justify-end">
                  <span
                    style={{
                      ...
                      background:
                        directionStyles[bestStock.direction].background,
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-4">
                <Stat
                  label="Pulse Score"
                  value={formatNumber(bestStock.momentum_pulse_score, 1)}
                  valueStyle={undefined}
                />
                <Stat
                  label="Trend Strength"
                  value={formatNumber(bestStock.pulse_trend_strength, 1)}
                  accent="#CFFAFE"
                />
                <Stat
                  label="Confidence"
                  value={`${formatNumber(bestStock.direction_confidence, 0)}%`}
                  accent={directionStyles[bestStock.direction].color}
                />
                <Stat
                  label="Day Change"
                  value={formatSignedPercent(bestStock.change_pct, 1)}
                  accent={bestStock.change_pct >= 0 ? "#86EFAC" : "#FCA5A5"}
                />
                <Stat
                  label="Rel Strength"
                  value={formatSignedPercent(bestStock.relative_strength, 1)}
                  accent={
                    bestStock.relative_strength >= 0 ? "#86EFAC" : "#FCA5A5"
                  }
                />
                <Stat
                  label="Trend"
                  value={bestStock.pulse_trend_label}
                  accent={directionStyles[bestStock.direction].color}
                />
              </div>
            ...
            >
              No best stock was returned for this sector yet.
            </div>
          )}
        </div>
