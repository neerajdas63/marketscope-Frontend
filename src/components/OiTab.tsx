...
function normalizeOptionSummary(
  ...
  return {
    ...
    top_resistances: Array.isArray(raw?.top_resistances)
      ? raw.top_resistances
      : [],
  };
}
...
function normalizeOiResponse(raw: any): OiApiResponse {
  ...
  const bankniftyRaw =
    raw?.banknifty ?? raw?.BANKNIFTY ?? raw?.indices?.banknifty ?? null;
  return {
  ...
  return (
    ...
    >
      <svg width="140" height="80" viewBox="0 0 140 85">
        ...
        <text x={cx} y={cy + 30} textAnchor="middle" fill="#aaa" fontSize="9">
          PCR {safe(pcr, 2)}
        </text>
      </svg>
    </div>
...
export function OiTab() {
  ...
  const filtered =
    signalFilter === "ALL"
      ? stocks
      : stocks.filter((s) => s.oi_signal === signalFilter);
  const sorted = [...filtered].sort(
    (a, b) => Math.abs(b.oi_change_pct) - Math.abs(a.oi_change_pct),
  );
  ...
  return (
    <div style={{ backgroundColor: "#0d0d0d", minHeight: "100vh" }}>
      ...
      >
        ...
        >
          <span style={{ color: "#555", fontSize: "11px", marginRight: "4px" }}>
            FILTER:
          </span>
          {(
            [
            ...
            <button
              key={f}
              style={signalFilter === f ? btnActive : btnBase}
            ...
            >
              ...
              >
                {f === "ALL"
                  ? stocks.length
                  : stocks.filter((s) => s.oi_signal === f).length}
              </span>
            </button>
          ))}
            ...
            <tbody>
              {loading ? (
                ...
                sorted.map((row, ri) => {
                  ...
                  const oiChgColor =
                    row.oi_change_pct >= 0 ? "#00C853" : "#FF1744";
                  const distColor =
                    Math.abs(row.distance_from_max_pain) <= 0.5
                      ? "#FFD600"
                      : row.distance_from_max_pain > 0
                        ? "#00C853"
                        : "#FF1744";
                  return (
                    <tr key={row.symbol}>
                      ...
                      >
                        ₹
                        {row.max_pain != null
                          ? row.max_pain.toLocaleString("en-IN")
                          : "--"}
                      </td>
                      <td
                        style={{
                      ...
                      >
                        {row.distance_from_max_pain >= 0 ? "+" : ""}
                        {safe(row.distance_from_max_pain, 2)}%
                      </td>
                      <td
                        style={{
                          ...tdStyle,
