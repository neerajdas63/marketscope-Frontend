...
const DIRECTION_OPTIONS: MomentumPulseDirectionFilter[] = [
  "ALL",
  "LONG",
  "SHORT",
];
const BEHAVIOR_STATE_ORDER: MomentumPulseBehaviorState[] = [
  "EARLY",
  "ACTIVE",
  "LATE",
  "EXTENDED",
...
const behaviorStateStyles: Record<
  MomentumPulseBehaviorState,
  { bg: string; border: string; color: string }
> = {
  EARLY: {
...
function StateBadge({
  ...
  state: MomentumPulseBehaviorState;
  timeContextBucket?: string;
}) {
  const style = behaviorStateStyles[state];
  const tooltip =
    timeContextBucket && timeContextBucket !== "--"
...
function MomentumPulseTopCard({ row }: { row: MomentumPulseRow }) {
  ...
  return (
    <div
      style={{
        background:
          "linear-gradient(135deg, rgba(14, 23, 38, 0.98), rgba(10, 15, 28, 0.96))",
    ...
    >
      ...
      <div className="flex items-center justify-between mt-3 gap-3">
        <div>
          ...
          <div
            style={{
              color: row.change_pct >= 0 ? "#4ADE80" : "#F87171",
              fontSize: "12px",
              fontWeight: 700,
            }}
          >
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ color: "#F8FAFC", fontSize: "20px", fontWeight: 800 }}>
            {formatNumber(row.momentum_pulse_score, 1)}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap mt-3">
        ...
        <TrendBadge
          label={row.pulse_trend_label}
          strength={row.pulse_trend_strength}
        />
        <StateBadge
          state={row.behavior_state}
          timeContextBucket={row.time_context_bucket}
        />
      </div>
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <DetailStat
          label="Volume Pace Score"
          value={formatNumber(row.volume_pace_score, 1)}
        />
        <DetailStat
          label="Range Expansion Score"
          value={formatNumber(row.range_expansion_score, 1)}
        />
        <DetailStat
          label="Long RS Score"
          value={formatNumber(row.long_relative_strength_score, 1)}
        />
        <DetailStat
          label="Short RS Score"
          value={formatNumber(row.short_relative_strength_score, 1)}
        />
        <DetailStat
          label="Long Consistency"
          value={formatNumber(row.long_directional_consistency_score, 1)}
        />
        <DetailStat
          label="Short Consistency"
          value={formatNumber(row.short_directional_consistency_score, 1)}
        />
        <DetailStat
          label="Long VWAP Align"
          value={formatNumber(row.long_vwap_alignment_score, 1)}
        />
        <DetailStat
          label="Short VWAP Align"
          value={formatNumber(row.short_vwap_alignment_score, 1)}
        />
        <DetailStat
          label="Today Cum Vol"
          value={formatCompactVolume(row.today_cum_volume)}
        />
        <DetailStat
          label="20D Same-Time Vol"
          value={formatCompactVolume(row.avg_20d_cum_volume_same_time)}
        />
        <DetailStat
          label="Intraday Range"
          value={formatNumber(row.intraday_range_abs, 2)}
        />
        <DetailStat
          label="Intraday Range %"
          value={formatPercent(row.intraday_range_pct)}
        />
        <DetailStat
          label="20D Range Abs"
          value={formatNumber(row.avg_20d_range_same_time_abs, 2)}
        />
        <DetailStat
          label="20D Range %"
          value={formatPercent(row.avg_20d_range_pct_same_time)}
        />
        <DetailStat
          label="Score Change 15m"
          value={formatPercent(row.score_change_15m, 1)}
        />
        <DetailStat
          label="Score Slope"
          value={formatNumber(row.score_slope, 2)}
        />
        <DetailStat
          label="Score Acceleration"
          value={formatNumber(row.score_acceleration, 2)}
        />
        <DetailStat
          label="Improving Streak"
          value={formatNumber(row.improving_streak, 0)}
        />
        <DetailStat
          label="Weakening Streak"
          value={formatNumber(row.weakening_streak, 0)}
        />
        <DetailStat label="VWAP" value={formatCurrency(row.vwap)} />
        <DetailStat
          label="Volume Surge"
          value={row.volume_surge ? "Yes" : "No"}
        />
        <DetailStat
          label="Range Expansion"
          value={row.range_expansion ? "Yes" : "No"}
        />
        <DetailStat
          label="Index Outperformer"
          value={row.index_outperformer ? "Yes" : "No"}
        />
        <DetailStat
          label="Trend Consistent"
          value={row.trend_consistent ? "Yes" : "No"}
        />
        <DetailStat
          label="Improving Now"
          value={row.improving_now ? "Yes" : "No"}
        />
      </div>
...
function DesktopRow({
  ...
  return (
    <>
      <TableRow
        className="border-b border-slate-800/80 bg-transparent hover:bg-slate-900/70 cursor-pointer"
        onClick={onToggle}
      >
        ...
        <TableCell className="py-3">
          <div
            ...
            style={{
              backgroundColor: directionTone.bg,
              border: `1px solid ${directionTone.border}`,
            }}
          >
            <span
            ...
            <span className="text-slate-300 text-[11px] font-semibold">
              {formatNumber(row.direction_confidence, 0)}
            </span>
          </div>
        </TableCell>
        ...
        <TableCell className="py-3 text-slate-300">
          {formatNumber(row.volume_pace_ratio, 2)}x
        </TableCell>
        <TableCell className="py-3 text-slate-300">
          {formatNumber(row.range_expansion_ratio, 2)}x
        </TableCell>
        <TableCell className="py-3">
          <span
            style={{
              color: row.relative_strength >= 0 ? "#4ADE80" : "#F87171",
              fontWeight: 700,
            }}
          >
        ...
        <TableCell className="py-3 text-slate-300">
          {formatPercent(row.score_change_5m, 1)}
        </TableCell>
        <TableCell className="py-3">
          <PulseSparkline row={row} />
        </TableCell>
        <TableCell className="py-3 text-slate-300">
          {formatPercent(row.distance_from_vwap_pct)}
        </TableCell>
        <TableCell className="py-3">
          <StateBadge
            state={row.behavior_state}
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div
              style={{ color: "#F8FAFC", fontSize: "18px", fontWeight: 800 }}
            >
              {row.symbol}
            </div>
            <div
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          ...
          <DetailStat
            label="Confidence"
            value={formatNumber(row.direction_confidence, 0)}
          />
          <DetailStat
            label="Pulse Score"
          ...
          <DetailStat
            label="Trend"
            value={`${row.pulse_trend_label} ${formatNumber(row.pulse_trend_strength, 1)}`}
          />
          <DetailStat
            label="Volume Pace"
            value={`${formatNumber(row.volume_pace_ratio, 2)}x`}
          />
          <DetailStat
            label="Range Exp"
            value={`${formatNumber(row.range_expansion_ratio, 2)}x`}
          />
          <DetailStat label="RS" value={formatPercent(row.relative_strength)} />
          <DetailStat
            label="Score 5m"
            value={formatPercent(row.score_change_5m, 1)}
          />
          <DetailStat
            label="Score 10m"
            value={formatPercent(row.score_change_10m, 1)}
          />
          <DetailStat
            label="Dist VWAP"
            value={formatPercent(row.distance_from_vwap_pct)}
          />
...
export function MomentumPulseTab() {
  ...
  const [query, setQuery] = useState<MomentumPulseQuery>({
    limit: 40,
    direction: "ALL",
    includeVeryWeak: false,
  });
  const [sortKey, setSortKey] = useState<SortKey>("default");
  const [data, setData] = useState<MomentumPulseResponse>(
    createEmptyMomentumPulseResponse(query),
  );
  ...
  const longCount = useMemo(
    () => sortedRows.filter((row) => row.direction === "LONG").length,
    [sortedRows],
  );
  const shortCount = useMemo(
    () => sortedRows.filter((row) => row.direction === "SHORT").length,
    [sortedRows],
  );
  const strongCount = useMemo(
    () => sortedRows.filter((row) => row.tier === "strong").length,
    [sortedRows],
  );
  const risingCount = useMemo(
    () => sortedRows.filter((row) => row.pulse_trend_label === "Rising").length,
    [sortedRows],
  );
      ...
      <div style={{ padding: "16px" }}>
        ...
        >
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              ...
              >
                Surface live momentum by behavior state, rising trend, and
                directional conviction without hiding late or extended names.
              </div>
            </div>
          ...
          <div className="flex gap-2 flex-wrap mt-4">
            <SummaryChip
              label="Total"
              value={String(data.total || sortedRows.length)}
            />
            <SummaryChip
              label="Benchmark"
          ...
          <div className="flex items-center gap-2 flex-wrap mt-4">
            <div className="flex items-center gap-1 p-1 rounded-xl border border-slate-800 bg-slate-950/60">
              {DIRECTION_OPTIONS.map((direction) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setQuery((current) => ({ ...current, direction }))
                  }
                  style={{
                    padding: "7px 11px",
                    borderRadius: "10px",
                    border: "none",
                    backgroundColor:
                      query.direction === direction ? "#0F766E" : "transparent",
                    color:
                      query.direction === direction ? "#F0FDFA" : "#94A3B8",
                    fontSize: "12px",
                    fontWeight: 700,
                  }}
                >
            ...
            <button
              ...
              onClick={() =>
                setQuery((current) => ({
                  ...current,
                  includeVeryWeak: !current.includeVeryWeak,
                }))
              }
              style={{
                padding: "8px 12px",
                ...
                backgroundColor: query.includeVeryWeak
            ...
            >
              ...
              <option value="range_expansion_ratio">
                Sort: Range Expansion
              </option>
            </select>
        ...
        {warmingUp ? (
          <div className="flex flex-col items-center justify-center gap-2 mt-4 rounded-2xl border border-slate-800 bg-slate-950/60 px-6 py-16 text-center">
            <div
              style={{ color: "#7DD3FC", fontSize: "18px", fontWeight: 700 }}
            >
              Momentum Pulse is warming up
            </div>
            <div
              style={{ color: "#94A3B8", fontSize: "13px", maxWidth: "560px" }}
            >
              The backend is reachable, but there are no live momentum names yet
              for the current filters. Try a manual refresh or widen the
              direction and very-weak settings.
            </div>
          </div>
        ) : null}
        ...
        {!warmingUp && sortedRows.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 mt-4 rounded-2xl border border-slate-800 bg-slate-950/60 px-6 py-16 text-center">
            ...
            >
              No momentum names match the current filters
            </div>
            <div style={{ color: "#94A3B8", fontSize: "13px" }}>
              Try switching direction, increasing the limit, or including very
              weak names.
            </div>
          </div>
        ) : null}
