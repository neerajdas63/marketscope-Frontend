...
function normalizePlan(p: any): TradePlan {
  ...
  return {
    ...
    entry_low:
      p.entry_zone_low ??
      p.entry_low ??
      (Array.isArray(p.entry_zone) ? p.entry_zone[0] : 0),
    entry_high:
      p.entry_zone_high ??
      p.entry_high ??
      (Array.isArray(p.entry_zone) ? p.entry_zone[1] : 0),
...
function PlanCard({ plan }: { plan: TradePlan }) {
  const stratCfg = STRATEGY_CFG[plan.strategy] ?? {
    bg: "#1a1a1a",
    color: "#888",
  };
    ...
    >
      ...
      >
        ...
        {plan.vwap != null && (
          <span style={{ color: "#888", fontSize: "11px" }}>
            VWAP{" "}
            <span
              style={{ color: plan.ltp >= plan.vwap ? "#69F0AE" : "#FF5252" }}
            >
              ₹{fmt(plan.vwap)}
            </span>
          </span>
        ...
        {plan.delivery_pct != null && (
          <span style={{ color: "#888", fontSize: "11px" }}>
            ...
            <span style={{ color: "#aaa" }}>
              {plan.delivery_pct.toFixed(0)}%
            </span>
          </span>
        )}
        <span style={{ marginLeft: "auto", color: "#666", fontSize: "11px" }}>
          RF: <span style={{ color: "#aaa" }}>{plan.rfactor_score}</span>
        </span>
      </div>
      ...
      {isAvoid ? (
        ...
        <>
          ...
          <div
            style={{
              backgroundColor: isShort ? "#1a0d0d" : "#0d1a0d",
              border: `1px solid ${isShort ? "#3a1a1a" : "#1a3a1a"}`,
              borderRadius: "6px",
              padding: "8px 12px",
            }}
          >
            ...
            >
              ENTRY ZONE{isShort ? " (SHORT — sell at high)" : ""}
            </div>
            <div
              style={{
                color: isShort ? "#FF8A80" : "#69F0AE",
                fontWeight: "bold",
                fontSize: "14px",
              }}
            >
          ...
          >
            <div>
              ...
              <span
                style={{ color: rrColor, fontWeight: "bold", fontSize: "12px" }}
              >
                1:{safe(plan.risk_reward, 1)}
              </span>
            </div>
            <div style={{ color: "#555", fontSize: "11px" }}>
              ORB H:{" "}
              <span style={{ color: "#FF9800" }}>₹{fmt(plan.orb_high)}</span>
              {"  "}L:{" "}
              <span style={{ color: "#42A5F5" }}>₹{fmt(plan.orb_low)}</span>
            </div>
          </div>
        </>
      )}
  ...
  return (
    <div
      style={{
        ...
        padding: "14px",
        height: "240px",
        animation: "plannerPulse 1.4s ease-in-out infinite",
      }}
    />
  );
}
    ...
    <div style={{ backgroundColor: "#0d0d0d", minHeight: "100vh" }}>
      ...
      >
        ...
        {(
          ...
          <button
            key={s}
            style={stratFilter === s ? btnActive : btnBase}
            ...
            style={
              confFilter === c
                ? {
                    ...btnActive,
                    ...(c === "HIGH"
                      ? { backgroundColor: "#1B5E20", borderColor: "#00C853" }
                      : c === "MEDIUM"
                        ? { backgroundColor: "#5a4400", borderColor: "#FFD600" }
                        : {}),
                  }
                : btnBase
            }
